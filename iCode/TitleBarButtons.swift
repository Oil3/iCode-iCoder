...
class TitleBarButtons: NSTitlebarAccessoryViewController {
    ...
    override func viewDidLoad() {
        ...
        transparencyButton = NSButton(image: NSImage(systemSymbolName: "eye.slash", accessibilityDescription: nil)!, target: self, action: #selector(toggleTransparency))
        transparencyButton.bezelStyle = .texturedRounded
        transparencyButton.isBordered = false
        ...
        toggleButton.frame = NSRect(x: 16, y: 0, width: 26, height: 26)
        transparencyButton.frame = NSRect(x: 40, y: 0, width: 26, height: 26)
        searchText.frame = NSRect(x: 70, y: 0, width: 98, height: 26)
    ...
    @objc func toggleTransparency() {
        FloatingController.shared.toggleTransparency()
        updateButtonAppearance()
    }
